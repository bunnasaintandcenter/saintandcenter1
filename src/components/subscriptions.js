...
const Subscriptions = ({ id }) => {
  ...
  const handleCancel = async (sub_id) => {
    ...
    const response = await axios.put(`https://andnone.co/saintcenter/wp-json/wc/v1/subscriptions/${sub_id}/?consumer_key=ck_990f62c74b9f424eb1ecf8b6b1bd3a2b7e180c7a&consumer_secret=cs_0c39f3c5f8db99d8f1493394fffadba7629215cd`, data)
  ...
  const renderStatus = (status, next_payment_date) => {
    switch (status) {
      case 'active':
        return <span>Renews on <Moment format='LL'>{next_payment_date}</Moment></span>
      case 'cancelled':
