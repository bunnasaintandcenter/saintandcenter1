...
it('renders', () => {
  const { getAllByTestId } = render(<ThemeProvider theme={theme}><Hero title='Hello World' /></ThemeProvider>)
  expect(getAllByTestId('hero')).toHaveLength(1)
});

it('renders the title', () => {
  const { getByTestId } = render(<ThemeProvider theme={theme}><Hero title='Hello World' /></ThemeProvider>)
